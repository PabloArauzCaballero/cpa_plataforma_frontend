# v29 - Parte de clases pasadas con selects desde sistema
...
- Se agregó carga real de catálogos desde sistema:
...
- Cuando se selecciona una materia del árbol, el formulario autocompleta `tema` y `subtema` cuando el sistema los devuelve.
- `Tema` y `Subtema` conservan edición manual porque el parte físico puede requerir detalle operativo más específico.
- El payload sigue enviando los campos esperados por el contrato del sistema, sin inventar campos adicionales.
