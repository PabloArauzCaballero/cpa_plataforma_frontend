...
export interface PersistentDraftPayload<T = unknown> {
  modulo: string;
  recurso: string;
...
export interface PersistentDraft<T = unknown> extends PersistentDraftPayload<T> {
...
const DRAFT_FRONTEND_VERSION = '1.1.37';
...
function asPersistentDraft<T>(record: CrudRecord): PersistentDraft<T> {
  return record as unknown as PersistentDraft<T>;
}
...
function getDraftTimestamp(draft: PersistentDraft): string {
  return String(draft.fecha_modificacion ?? draft.fecha_registro ?? '');
}

export function buildPersistentDraftKey(resource: CrudResourceDefinition, operation: DraftOperation, recordId?: string | number | null): string {
  return buildDraftBaseKey(resource, operation, recordId);
}

export function buildPersistentDraftPayload<T>(
...
export async function listPersistentDrafts<T>(resource: CrudResourceDefinition, operation: DraftOperation, recordId?: string | number | null): Promise<Array<PersistentDraft<T>>> {
  ...
  const rows = normalizeListResponse(response).map(asPersistentDraft<T>);
  return rows
...
export async function getLatestPersistentDraft<T>(resource: CrudResourceDefinition, operation: DraftOperation, recordId?: string | number | null): Promise<PersistentDraft<T> | null> {
  const drafts = await listPersistentDrafts<T>(resource, operation, recordId ?? null);
  return drafts[0] ?? null;
}

export async function savePersistentDraft<T>(
  resource: CrudResourceDefinition,
  operation: DraftOperation,
  payload: T,
...
): Promise<PersistentDraft<T>> {
  const draftPayload = buildPersistentDraftPayload(resource, operation, payload, {
  ...
  const response = draftId
    ? await httpClient.patch<unknown, Partial<PersistentDraftPayload<T>>>(`${DRAFT_ENDPOINT}/${draftId}`, draftPayload)
    : await httpClient.post<unknown, PersistentDraftPayload<T>>(DRAFT_ENDPOINT, draftPayload);

  return asPersistentDraft<T>(normalizeRecordResponse(response));
}

export async function discardPersistentDraft(draftId: string | number): Promise<void> {
  await httpClient.patch<unknown, CrudRecord>(`${DRAFT_ENDPOINT}/${draftId}`, { estado_borrador: 'DESCARTADO' });
}

export function readDraftPayload<T>(draft: PersistentDraft<T> | null): T | null {
...
export function readDraftIds(draft: PersistentDraft | null): CrudRecord {
